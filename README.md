...
`describe()` blocks keeps the output of the unit tests organized and allows for multiple tests to be nested in the block (like having a folder). `it()` begins the actual test block and states what the function/module is doing.

2. What is the point of `Test Driven Development`? What do you think about this approach?

The point of TDD is to handle features in a succinct, organized, thoughtful manner.  We first start by writing a test (that fails), then write just enough code for the test to pass, refactor, and then commit/implement the feature. I think it’s a helpful way to stay organized and handle feature implementation in steps rather than all at once.
